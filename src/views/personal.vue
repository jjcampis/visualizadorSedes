      ...
      <v-col cols="12" md="12">
        <b-collapse v-model="psVisible" visible id="collapse-2" class="mt-2">
          <div class="d-flex justify-content-between">
            <v-btn-toggle v-model="fTipo" rounded>
              <v-btn value="Todos">
                <v-icon :class="{ 'text-white': fTipo == 'Todos' }"
                  >mdi-sigma</v-icon
                >
                Todos
              </v-btn>
              <v-btn value="Pedagógico">
                <v-icon :class="{ 'text-white': fTipo == 'Pedagógico' }"
                  >mdi-school</v-icon
                >
                pedagogicos
              </v-btn>
              <v-btn value="Programador">
                <v-icon :class="{ 'text-white': fTipo == 'Programador' }"
              ...
              <v-btn value="Técnico">
                <v-icon :class="{ 'text-white': fTipo == 'Técnico' }">
            ...
            <div
              v-if="
                personalFTipo != null &&
                Object.entries(personalFTipo).length > 0
              "
              style="display: flex; flex-grow: 1"
            >
              <v-badge overlap color="medalla" :content="personalFTipo.length">
              </v-badge>
            </div>
            ...
            <v-btn-toggle v-model="fLA" rounded>
              <v-btn value="Todos">
                <v-icon :class="{ 'text-white': fLA == 'Todos' }"
                  >mdi-sigma</v-icon
                >
                Todos
              </v-btn>
          ...
          <b-table-simple
            class="b-table-sticky-header-ac"
            sticky-header
            striped
            small
            responsive
              ...
              <b-tr v-for="(pers, index) in personalFTipo" :key="index">
                ...
                <b-td>
                  ...
                  <v-tooltip v-if="pers.ps_rol == 'Facilitador de Sede'" bottom>
                    <template v-slot:activator="{ on, attrs }">
                      <span v-bind="attrs" v-on="on" class="FC"> FC </span>
                    </template>
                  ...
                  <v-tooltip v-if="pers.ps_rol == 'Referente de Sede'" bottom>
                    <template v-slot:activator="{ on, attrs }">
                      <span v-bind="attrs" v-on="on" class="RS"> RS </span>
                    </template>
                    <span>Referente de sede</span>
                  </v-tooltip>

                  <v-tooltip
                    v-if="pers.ps_rol == 'Referente de Administrativo'"
                    bottom
                  >
                    <template v-slot:activator="{ on, attrs }">
                      <span v-bind="attrs" v-on="on" class="RA"> RA </span>
                    </template>
                ...
                <b-td style="min-width: 152px">
                  <div v-if="Object.entries(pers).length > 0">
                    <span
                      v-for="(perfil, index) in pers.ps_perfil.split(',')"
                      :key="'perf' + index"
                    >
                      <p class="iconos-rm">
                ...
                <b-td style="min-width: 152px">
                  <span
                    v-for="(LA, index) in pers.ps_linea_de_accion.split(',')"
                    :key="'LA' + index"
                  >
                    <p class="iconos-rm">
...
export default {
  ...
  data: () => {
    return {
      ...
      showcharts: false,
      cambio: false,
      //dpclick: false, //SE USA PARA EL CLICK DE DATAPOINT
      cantP: {},
      sede: "Todo ",
      psVisible: true,
      renderedperso: 0,
  ...
  computed: {
    ...mapState(["personal", "sedes", "rubricas_G","selectedSede"]),
    menux() {
      ...
      if (this.sede != "Todo") {
        if (this.personal.length > 0) {
          ...
          return this.personal.filter((tray) => {
            ...
            return tray.ps_espacio_maker == this.selectedSede.sede;
          });
        } else {
          return ["no habia"];
        }
    ...
    personalFTipo() {
      if (this.personalF != null && Object.entries(this.personalF).length > 0) {
        if (
          (this.fTipo != undefined && this.fTipo != "Todos") ||
          (this.fLA != undefined && this.fLA != "Todos")
        ) {
          if (this.fTipo != undefined && this.fTipo != "Todos") {
            var filtro = this.personalF.filter((pers) =>
              pers.ps_perfil.includes(this.fTipo)
            );
            //return filtro
          } else {
            //no hay ningun filtro o son todos
            filtro = this.personalF;
          }
          if (this.fLA != undefined && this.fLA != "Todos") {
            if (filtro.length > 0) {
              return filtro.filter((pers) =>
                pers.ps_linea_de_accion.includes(this.fLA)
              );
            }
          }
          return filtro;
    ...
    cant_horasP() {
      if (this.personal.length > 0) {
        ...
        //return agrupadohp;
        let suma = Object.values(agrupadohp)
          .map((x) => {
            return x;
          })
          .map((x) => x.map((y) => y.ps_carga_horaria_total));
        let reducex = suma.map((x) =>
          x.reduce((a, b) => parseFloat(a) + parseFloat(b), 0)
        );
        return reducex;
      }
    },
    cant_pers() {
      ...
      if (this.personal.length > 0) {
        ...
        let cp = this.sedes.map(
          (x) =>
            (this.cantP[x.sede] = this.personal.filter((sede) => {
  ...
  methods: {
    ordenar(vec) {
      ...
      let cp = this.sedes.map(
        (x) =>
          (temp[x.sede] = this.personal.filter((sede) => {
            return sede.ps_espacio_maker == x.sede;
          }))
      );
   ...
   actuperso(){
        ...
        //setTimeout(() => {this.showchartsH = true;},50),
  ...
  watch: {
    ...
    //pinta igual la seleccionada
    selectedSede(newVal) {
            if (newVal) {
              this.pintar(newVal);
              }
          },
    //observo el boton del collapsible
    estado(newVal){
          if (newVal) {
              this.actuperso();
              }
